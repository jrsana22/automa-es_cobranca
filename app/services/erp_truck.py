"""
Client específico para o ERP APVS Truck (erp.apvstruck.org.br).
Mesmo sistema base ASP.NET WebForms que o APVS Brasil.
Diferenças: tema #28bebe, eng_chave=cd_empresa=N, historico="Auto Impressão de Relatórios"

Fluxo real:
1. GET /WebClient.aspx → inicializa sessão + cookies
2. POST /default.aspx/GravarVisitorID → grava fingerprint
...
6. GET tela de relatório → carrega formulário com campos hidden
7. POST /Excel.aspx → retorna XLSX
"""
...
# Diferenças do Truck vs Brasil
ID_TEMA = "#28bebe"  # Brasil usa #004182
ID_HISTORICO = " Auto Impressão de Relatórios"  # Brasil usa " Auto Impressão de 2ª Via"

class PVSTruckClient(BaseERPClient):
    """Client HTTP para o ERP APVS Truck (ASP.NET WebForms)."""

    def __init__(self, base_url: str, login: str, senha: str):
        super().__init__(base_url, login, senha)
        self._visitor_id = f"fp_truck_{uuid.uuid4().hex[:12]}"
    ...
    def login(self) -> bool:
        ...
        Mesmo fluxo do APVS Brasil (mesmo sistema base).
        """
        ...
        if password_field:
            ...
            logger.error(
                f"Login Truck falhou — página de login ainda presente. "
        ...
        if "WebClient.aspx" in resp.url:
            ...
            logger.error(
                f"Login Truck falhou — redirecionado para WebClient. "
                f"URL: {resp.url} | Snippet: {snippet[:200]}"
            )
            return False

        logger.info(f"Login APVS Truck realizado com sucesso. URL final: {resp.url}")
        return True
    ...
    def exportar_inadimplencia(
        ...
        export_data = {
            ...
            "eng_validade_eis": "0",
            "eng_chave": f"cd_empresa={self._cd_empresa}" if self._cd_empresa else "id_processo=0",
